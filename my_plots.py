...
def name_trend_plot(df, name='John', width=800, height=600):
    ...
    if name_data.empty:
        ...
        # Create a single plot for the sex ratios
        fig = go.Figure()

        # Add Male Ratio
        fig.add_trace(go.Scatter(
            x=yearly_counts['year'], 
            y=yearly_counts['Male_Ratio'], 
            mode='lines',
            name='Male Ratio',
            line=dict(color=color_map['M']),
            hovertemplate='Year: %{x}<br>Male Ratio: %{y:.2f}<extra></extra>'
        ))

        # Add Female Ratio
        fig.add_trace(go.Scatter(
            x=yearly_counts['year'], 
            y=yearly_counts['Female_Ratio'], 
            mode='lines',
            name='Female Ratio',
            line=dict(color=color_map['F']),
            hovertemplate='Year: %{x}<br>Female Ratio: %{y:.2f}<extra></extra>'
        ))

        # Update layout
        fig.update_layout(
            title=f"Sex Ratios Over Time for '{name}'",
            xaxis_title="Year",
            yaxis_title="Ratio",
            yaxis=dict(range=[0, 1]),  # Ratio should always be between 0 and 1
            legend_title="Legend",
            width=width,
            height=height,
